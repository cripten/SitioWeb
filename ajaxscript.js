...
jQuery.fn.vtexSmartResearch=function(opts)
{
    ...
    var defaults=
	{
		...
		emptySearchMsg:'<h3>Esta combinacion de filtros no arroja ningun resultado!</h3>', // Html com a mensagem para ser apresentada quando não existirem resultados para os filtros selecionados
		...
		popupAutoCloseSeconds:0, // Caso esteja utilizando popup, defina aqui o tempo para que ele feche automaticamente
	...
	var fn=
	{
		...
		changeSearchUrl:function()
		{
        const regex = /&O=(.*?)&/g
				filtro=$("#ordenarpor").val()
          if (regex.exec(currentSearchUrl) !== null) {
              console.log("case 1");
              currentSearchUrl = currentSearchUrl.replace(/&O=(.*?)&/g, filtro)
          } else {
              console.log("case 2");
              currentSearchUrl = currentSearchUrl+filtro.slice(0, -1)
          }
	},
		...
		{
				console.log(options.authorizeScroll(ajaxCallbackObj))
		...
		allScroll:function()
		{
			console.log("la cantidad es" +options.pageLimit)
			var elementPages,pages,currentStatus,tmp;

			elementPages=body.find(".pager:first").attr("id");
			tmp=(elementPages||"").split("_").pop();
			pages=options.pageLimit;
			currentStatus=true;
	...
	var currentUrl=document.location.href,
		...
		prodOverlay1=jQuery('<div class="vtexSr-overlay1"></div>'),
	...
	filtersMenuE.before(prodOverlay1);
	...
	var fns=
	{
		exec:function()
		{
			...
			$("#ordenarpor").bind("change",function(){
				fns.changeFilter();
